package com.anwesh.uiprojects.rightangletovertlineview

/**
 * Created by anweshmishra on 07/01/19.
 */
...
fun Canvas.drawRAVLNode(i : Int, scale : Float, paint : Paint) {
...
class RightAngledToVertLineView(ctx : Context) : View(ctx) {
    ...
    data class State(var scale : Float = 0f, var prevScale : Float = 0f, var dir : Float = 0f) {
        ...
        fun update(cb : (Float) -> Unit) {
            scale += scale.updateScale(dir, 1, 1)
    ...
    data class RAVLNode(var i : Int, val state : State = State()) {

        private var next : RAVLNode? = null
        private var prev : RAVLNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRAVLNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RAVLNode {
            var curr : RAVLNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
